...
<div class="space-y-6" x-data="{ modalTambah: false }">
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-50" x-data="{ modalDetail: false, modalEdit: false, modalHapus: false }">
                    ...
                    <td class="p-3 text-center whitespace-nowrap">
                        ...
                        <!-- Tombol Edit (Sekaligus mengatur akses) -->
                        ...
                        <div x-show="modalDetail" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 text-left" style="display: none;">
                            <div class="card-container max-w-sm w-full shadow-xl" @click.away="modalDetail = false">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div>
                                        <label class="form-label text-gray-500">Hak Akses yang Dimiliki:</label>
                                        <ul class="list-disc list-inside mt-2 text-xs text-gray-700 bg-gray-50 p-3 rounded border max-h-32 overflow-y-auto">
                                            @forelse($item->permissions as $perm)
                                                <li>{{ $perm->display_name }}</li>
                                            @empty
                                                <li class="italic text-gray-400">Tidak ada hak akses</li>
                                            @endforelse
                                        </ul>
                                    </div>
                                </div>
                        ...
                        <div x-show="modalEdit" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 text-left" style="display: none;">
                            <div class="card-container max-w-lg w-full shadow-xl" @click.away="modalEdit = false">
                                <h3 class="text-lg font-bold text-gray-800 mb-2 border-b pb-2">Edit Role & Hak Akses</h3>
                                <p class="text-xs text-gray-500 mb-4">Ubah nama role atau atur izin fitur yang bisa diakses.</p>
                                ...
                                <form action="{{ route('admin.role.update', $item->id_role) }}" method="POST" class="space-y-4">
                                    ...
                                    <div>
                                        <label class="form-label text-xs font-bold text-gray-700 mb-1 block">Atur Hak Akses (Permissions)</label>
                                    ...
                                    <div class="flex justify-end gap-2 mt-6 border-t pt-4">
                                        ...
                                        <button type="submit" class="btn-action-edit px-4 py-2">Simpan Perubahan</button>
                                    </div>
                                </form>
                            </div>
                        </div>
    ...
    <!-- Modal Tambah (Dengan Izin Akses) -->
    <div x-show="modalTambah" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 text-left" style="display: none;">
        <div class="card-container max-w-lg w-full shadow-xl" @click.away="modalTambah = false">
            ...
            <p class="text-xs text-gray-500 mb-4">Masukkan nama role baru dan tentukan hak akses awalnya di bawah ini.</p>
