...
<div class="space-y-6" x-data="{ modalTambah: false, selected: [], selectAll: false }">
    ...
    <div class="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center bg-white p-5 rounded-xl shadow-sm border border-gray-100 relative z-20">
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
            ...
            <form action="{{ route('admin.level2.index') }}" method="GET" class="flex flex-row gap-2 w-full sm:w-auto">
                <div x-data="{
                    open: false,
                    searchQuery: '',
                    selectedValue: '{{ request('search') }}',
                    selectedLabel: '{{ request('search') ?: 'Semua Kegiatan' }}',
                    options: [
                        { value: '', label: 'Semua Kegiatan' },
                        @foreach($kegiatan->pluck('nama_kegiatan')->unique() as $k)
                    ...
                    <div x-show="open" @click.away="open = false" class="absolute z-50 w-full mt-1.5 bg-white border border-gray-200 rounded-lg shadow-xl" style="display: none;" x-transition.opacity>
                        <div class="p-2 border-b border-gray-100">
                            <input type="text" x-model="searchQuery" placeholder="Cari Kegiatan..." class="w-full px-3 py-2 border border-gray-200 rounded-md text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" autocomplete="off" @click.stop>
                        </div>
                        <div class="max-h-60 overflow-y-auto p-1.5 space-y-0.5">
                            ...
                            <div x-show="filteredOptions.length === 0" class="px-3 py-2 text-sm text-gray-400 italic text-center">Kegiatan tidak ditemukan</div>
                        </div>
                    </div>
                </div>
                ...
                @if(request('search'))
                    <a href="{{ route('admin.level2.index') }}" class="bg-gray-100 text-gray-600 px-3 py-2.5 rounded-lg font-bold text-sm hover:bg-gray-200 flex items-center shrink-0">Reset</a>
                @endif
            </form>

            <form action="{{ route('admin.level2.bulkDestroy') }}" method="POST" x-show="selected.length > 0" onsubmit="return confirm('Yakin ingin menghapus semua data yang dicentang? Semua data di bawahnya (Level 3 & 4) bisa ikut terhapus.');" class="w-full sm:w-auto" style="display: none;">
                @csrf @method('DELETE')
            ...
            <table class="w-full text-left border-collapse min-w-[600px]">
                <thead>
                    <tr class="bg-slate-50/80 border-b border-gray-100 text-slate-500 text-[11px] uppercase tracking-wider">
                        <th class="py-4 px-4 w-12 text-center"><input type="checkbox" x-model="selectAll" @change="selected = selectAll ? {{ json_encode($kegiatan->pluck('id_kegiatan')->map(fn($id) => (string)$id)) }} : []" class="w-4 h-4 text-[#14B8A6] border-gray-300 rounded cursor-pointer"></th>
                        <th class="py-4 px-4 w-16 text-center font-bold">No</th>
                        <th class="py-4 px-4 font-bold">Nama Kegiatan</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors group" x-data="{ modalDetail: false, modalEdit: false, modalHapus: false }">
                        <td class="py-4 px-4 text-center"><input type="checkbox" x-model="selected" value="{{ $item->id_kegiatan }}" class="w-4 h-4 text-[#14B8A6] border-gray-300 rounded cursor-pointer"></td>
                        <td class="py-4 px-4 text-center text-gray-400 font-semibold text-sm">{{ $index + 1 }}</td>
                        
                        <!-- Nama Kegiatan & Output -->
                        <td class="py-4 px-4">
                            <div class="flex flex-col justify-center">
                                <span class="text-sm font-bold text-gray-800">{{ $item->nama_kegiatan }}</span>
                                @if($item->output && $item->output->nama_output)
                                    <span class="text-xs font-medium text-gray-500 mt-1">{{ $item->output->nama_output }}</span>
                                @endif
                            </div>
                        </td>
                        ...
                        <td class="py-4 px-4 text-center whitespace-nowrap">
                            ...
                            <div x-show="modalDetail" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] p-4 text-left" style="display: none;" x-transition>
                                <div class="bg-white rounded-2xl max-w-md w-full shadow-xl p-6" @click.away="modalDetail = false" x-transition.scale>
                                    <h3 class="text-base font-bold text-gray-800 mb-4 border-b border-gray-100 pb-3">Detail Kegiatan Level 2</h3>
                                    <div class="space-y-4 text-sm">
                                        <div><label class="block text-xs font-bold text-gray-500 mb-1">Nama Kegiatan</label>
                                            <div class="w-full bg-gray-50 border border-gray-100 rounded-lg p-3 font-semibold text-gray-800">
                                                {{ $item->nama_kegiatan }}
                                            </div>
                                        </div>
                                        <div><label class="block text-xs font-bold text-gray-500 mb-1">Output Induk</label>
                                            <div class="w-full bg-gray-50 border border-gray-100 rounded-lg p-3 font-semibold text-gray-800">
                                                {{ $item->output->nama_output ?? '-' }}
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div x-show="modalEdit" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] p-4 text-left" style="display: none;" x-transition>
                                <div class="bg-white rounded-2xl max-w-md w-full shadow-xl p-6" @click.away="modalEdit = false" x-transition.scale>
                                    <h3 class="text-base font-bold text-gray-800 mb-4 border-b border-gray-100 pb-3">Edit Kegiatan Level 2</h3>
                                    <form action="{{ route('admin.level2.update', $item->id_kegiatan) }}" method="POST" class="space-y-4 text-sm">
                                        @csrf @method('PUT')
                                        
                                        <div>
                                            <label class="block text-xs font-bold text-gray-700 mb-1">Pilih Output Induk</label>
                                            
                                            <!-- Dropdown Edit Bisa Diketik -->
                                            <div x-data="{
                                                openOut: false,
                                                searchOut: '',
                                                selectedOutValue: '{{ $item->id_output ?? '' }}',
                                                selectedOutLabel: '{{ $item->output ? addslashes($item->output->nama_output) : '-- Pilih Output --' }}',
                                                outputs: [
                                                    { value: '', label: '-- Pilih Output --' },
                                                    @foreach($outputs as $out)
                                                    { value: '{{ $out->id_output }}', label: '{{ addslashes($out->nama_output) }}' },
                                                    @endforeach
                                                ],
                                                get filteredOutputs() {
                                                    if (this.searchOut === '') return this.outputs;
                                                    return this.outputs.filter(o => o.label.toLowerCase().includes(this.searchOut.toLowerCase()));
                                                },
                                                selectOut(opt) {
                                                    this.selectedOutValue = opt.value;
                                                    this.selectedOutLabel = opt.label;
                                                    this.searchOut = '';
                                                    this.openOut = false;
                                                }
                                            }" class="relative w-full z-40">
                                                <input type="hidden" name="id_output" :value="selectedOutValue" required>
                                                
                                                <button type="button" @click="openOut = !openOut" class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm bg-white flex justify-between items-center focus:outline-none focus:ring-2 focus:ring-amber-500 shadow-sm transition-colors">
                                                    <span x-text="selectedOutLabel" class="truncate text-gray-700"></span>
                                                    <svg class="w-4 h-4 text-gray-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                                                </button>

                                                <div x-show="openOut" @click.away="openOut = false" class="absolute w-full mt-1.5 bg-white border border-gray-200 rounded-lg shadow-xl" style="display: none;" x-transition.opacity>
                                                    <div class="p-2 border-b border-gray-100">
                                                        <input type="text" x-model="searchOut" placeholder="Cari Output..." class="w-full px-3 py-2 border border-gray-200 rounded-md text-sm focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500" autocomplete="off" @click.stop>
                                                    </div>
                                                    <div class="max-h-48 overflow-y-auto p-1.5 space-y-0.5">
                                                        <template x-for="opt in filteredOutputs" :key="opt.value">
                                                            <div @click="selectOut(opt)" 
                                                                 class="px-3 py-2 cursor-pointer text-sm rounded-md transition-colors"
                                                                 :class="selectedOutValue === opt.value ? 'bg-amber-50 text-amber-700 font-bold' : 'text-gray-700 hover:bg-gray-100'">
                                                                <span x-text="opt.label"></span>
                                                            </div>
                                                        </template>
                                                        <div x-show="filteredOutputs.length === 0" class="px-3 py-2 text-sm text-gray-400 italic text-center">Output tidak ditemukan</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <input type="text" name="nama_kegiatan" value="{{ $item->nama_kegiatan }}" required class="w-full border border-gray-200 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-amber-500">
                                        </div>
    ...
    <div x-show="modalTambah" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] p-4 text-left" style="display: none;" x-transition>
        <div class="bg-white rounded-2xl max-w-md w-full shadow-xl p-6" @click.away="modalTambah = false" x-transition.scale>
            <div class="flex justify-between items-center border-b border-gray-100 pb-3 mb-4">
                <h3 class="text-base font-bold text-gray-800">Tambah Kegiatan Level 2</h3>
                <button type="button" @click="modalTambah = false" class="text-gray-400 hover:text-gray-600 font-bold text-xl">&times;</button>
            </div>
            
            <form action="{{ route('admin.level2.store') }}" method="POST" class="space-y-4 text-sm">
                @csrf
                <div>
                    <label class="block text-xs font-bold text-gray-700 mb-1">Pilih Output Induk</label>
                    ...
                    <div x-data="{
                        ...
                        <button type="button" @click="openOut = !openOut" class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm bg-white flex justify-between items-center focus:outline-none focus:ring-2 focus:ring-emerald-500 shadow-sm transition-colors">
                            <span x-text="selectedOutLabel" class="truncate text-gray-700"></span>
                            <svg class="w-4 h-4 text-gray-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                        </button>
                        ...
                        <div x-show="openOut" @click.away="openOut = false" class="absolute w-full mt-1.5 bg-white border border-gray-200 rounded-lg shadow-xl" style="display: none;" x-transition.opacity>
                            <div class="p-2 border-b border-gray-100">
                                <input type="text" x-model="searchOut" placeholder="Cari Output..." class="w-full px-3 py-2 border border-gray-200 rounded-md text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" autocomplete="off" @click.stop>
                            </div>
                            <div class="max-h-48 overflow-y-auto p-1.5 space-y-0.5">
                                <template x-for="opt in filteredOutputs" :key="opt.value">
                                    <div @click="selectOut(opt)" 
                                         class="px-3 py-2 cursor-pointer text-sm rounded-md transition-colors"
                                         :class="selectedOutValue === opt.value ? 'bg-emerald-50 text-emerald-700 font-bold' : 'text-gray-700 hover:bg-gray-100'">
                                        <span x-text="opt.label"></span>
                                    </div>
                                </template>
                                <div x-show="filteredOutputs.length === 0" class="px-3 py-2 text-sm text-gray-400 italic text-center">Output tidak ditemukan</div>
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-700 mb-1">Nama Kegiatan</label>
                    <input type="text" name="nama_kegiatan" required class="w-full border border-gray-200 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                </div>
