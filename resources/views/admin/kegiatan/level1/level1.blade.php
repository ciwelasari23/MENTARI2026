...
<div class="space-y-6" x-data="{ modalTambah: false, selected: [], selectAll: false }">
    ...
    <div class="flex flex-wrap gap-4 justify-between items-center card-container">
        ...
        <div class="flex items-center gap-2">
            ...
            <form action="{{ route('admin.level1.index') }}" method="GET" class="flex gap-2 mr-4">
                ...
                @if(request('search'))
                    <a href="{{ route('admin.level1.index') }}" class="bg-gray-200 text-gray-700 px-3 py-2 rounded-lg font-bold text-sm hover:bg-gray-300 flex items-center">Reset</a>
                @endif
            </form>
            ...
            <thead>
                <tr class="bg-gray-50 border-b text-gray-600">
                    <th class="p-3 w-10 text-center">
                        <input type="checkbox" x-model="selectAll" @change="selected = selectAll ? {{ json_encode($outputs->pluck('id_output')->map(fn($id) => (string)$id)) }} : []" class="w-4 h-4 text-[#14B8A6] border-gray-300 rounded cursor-pointer">
                    </th>
                    <th class="p-3 w-16 text-center">No</th>
                    <th class="p-3">Nama Output</th>
                    <th class="p-3">Tahun</th>
                    <th class="p-3">Tim Kerja</th>
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-50" x-data="{ modalEdit: false, modalDetail: false, modalHapus: false }">
                    <td class="p-3 text-center">
                        <input type="checkbox" x-model="selected" value="{{ $item->id_output }}" class="w-4 h-4 text-[#14B8A6] border-gray-300 rounded cursor-pointer">
                    </td>
                    <td class="p-3 text-center">{{ $index + 1 }}</td>
                    <td class="p-3 font-semibold text-gray-800">{{ $item->nama_output }}</td>
                    ...
                    <td class="p-3 text-center whitespace-nowrap">
                        ...
                        <div x-show="modalDetail" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 text-left" style="display: none;">
                            <div class="card-container max-w-md w-full shadow-xl" @click.away="modalDetail = false">
                                <h3 class="text-lg font-bold text-gray-800 mb-4 border-b pb-2">Detail Output Level 1</h3>
                                <div class="space-y-4">
                                    <div><label class="form-label">Nama Output</label><div class="form-input bg-gray-50">{{ $item->nama_output }}</div></div>
                                    <div><label class="form-label">Tahun</label><div class="form-input bg-gray-50">{{ $item->tahun }}</div></div>
                                    <div><label class="form-label">Tim Kerja</label><div class="form-input bg-gray-50">{{ $item->team->nama_team ?? 'Tanpa Tim Kerja' }}</div></div>
                                </div>
                        ...
                        <div x-show="modalEdit" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 text-left" style="display: none;">
                            <div class="card-container max-w-md w-full shadow-xl" @click.away="modalEdit = false">
                                ...
                                <form action="{{ route('admin.level1.update', $item->id_output) }}" method="POST" class="space-y-4">
                                    @csrf @method('PUT')
                                    <div><label class="form-label">Nama Output</label><input type="text" name="nama_output" value="{{ $item->nama_output }}" required class="form-input"></div>
                                    <div><label class="form-label">Tahun</label><input type="number" name="tahun" value="{{ $item->tahun }}" required class="form-input"></div>
                                    <div><label class="form-label">Tim Kerja</label>
                                        <select name="id_team" class="form-input">
                                            <option value="">-- Tanpa Tim Kerja --</option>
                                            @foreach($teams as $team)
                                                <option value="{{ $team->id_team }}" {{ $item->id_team == $team->id_team ? 'selected' : '' }}>{{ $team->nama_team }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                ...
                <tr>
                    <td colspan="6" class="p-6 text-center text-gray-400">
                        ...
                        @else
                            Belum ada data output kegiatan (Level 1).
                        @endif
                    </td>
                </tr>
                @endforelse
    ...
    <div x-show="modalTambah" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" style="display: none;">
        <div class="card-container max-w-md w-full shadow-xl" @click.away="modalTambah = false">
            ...
            <form action="{{ route('admin.level1.store') }}" method="POST" class="space-y-4">
                @csrf
                <div><label class="form-label">Nama Output</label><input type="text" name="nama_output" required class="form-input"></div>
                <div><label class="form-label">Tahun</label><input type="number" name="tahun" required value="{{ date('Y') }}" class="form-input"></div>
                <div><label class="form-label">Tim Kerja</label>
                    <select name="id_team" class="form-input">
                        <option value="">-- Tanpa Tim Kerja --</option>
                        @foreach($teams as $team)
